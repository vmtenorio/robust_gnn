...
class GCNModel2(nn.Module):
    def __init__(self, S, in_dim, hid_dim, out_dim, n_layers, bias=True,
                 act=nn.ReLU, last_act=nn.Identity, dropout=0, batch_norm=False):
        super().__init__()
        ...
        self.convs = nn.ModuleList()
        if n_layers > 1:
            self.convs.append(GCNLayerM2(self.S.data, in_dim, hid_dim, bias))
            if self.batch_norm:
                self.batch_norm_layers.append(nn.BatchNorm1d(hid_dim))
            for _ in range(n_layers - 2):
                self.convs.append(GCNLayerM2(self.S.data, hid_dim, hid_dim, bias))
                if self.batch_norm:
                    self.batch_norm_layers.append(nn.BatchNorm1d(hid_dim))
            self.convs.append(GCNLayerM2(self.S.data, hid_dim, out_dim, bias))
        else:
            self.convs.append(GCNLayerM2(self.S.data, in_dim, out_dim, bias))
    ...
    def commutativity_term(self):
        ...
        for layer in self.convs:
            commut += torch.linalg.norm(layer.H @ self.S - self.S @ layer.H, 'fro')**2
        return commut
